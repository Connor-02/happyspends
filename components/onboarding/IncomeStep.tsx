...
const PRESETS = [
  { name: 'Job / Salary', icon: '💼', freq: 'fortnightly' as Frequency },
  { name: 'Side Hustle', icon: '🚀', freq: 'monthly' as Frequency },
  { name: 'Centrelink', icon: '🏛️', freq: 'fortnightly' as Frequency },
  { name: 'Freelance', icon: '💻', freq: 'monthly' as Frequency },
  { name: 'Rental Income', icon: '🏠', freq: 'monthly' as Frequency },
  { name: 'Other Income', icon: '💵', freq: 'monthly' as Frequency },
];
...
export function IncomeStep({
  ...
  function save() {
    ...
    onChange([
      ...incomeSources,
      {
        ...
        nextPayDate: form.nextPayDate || undefined,
      },
    ]);
    setForm(null);
  }
  ...
  function remove(id: string) {
    onChange(incomeSources.filter((s) => s.id !== id));
  }
  ...
  return (
    <OnboardingStepCard
      ...
      subtitle="Add your income sources so we can see what you have to work with."
      onNext={onNext}
      onBack={onBack}
      onSkip={onSkip}
    ...
    >
      <div className="space-y-4">
        ...
        <AnimatePresence>
          {form && (
            <motion.div
              ...
              exit={{ opacity: 0, y: -8 }}
              className="bg-white dark:bg-gray-800 rounded-3xl border-2 border-pink-300 dark:border-pink-700 p-4 space-y-3 shadow-sm"
            >
              <div className="flex items-center justify-between">
                <p className="font-bold text-gray-800 dark:text-white">New Income Source</p>
                <button
              ...
              <input
                autoFocus
                type="text"
                placeholder="Name (e.g. Salary at Woolworths)"
                value={form.name}
                onChange={(e) => setField('name', e.target.value)}
                className="w-full px-4 py-3.5 rounded-xl border-2 border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-900 text-gray-900 dark:text-white focus:outline-none focus:border-pink-400 transition-colors"
              />
              ...
              <div className="flex gap-2">
                <div className="relative flex-1">
                  ...
                  <input
                    ...
                    className="w-full pl-8 pr-3 py-3.5 rounded-xl border-2 border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-900 text-gray-900 dark:text-white focus:outline-none focus:border-pink-400 transition-colors"
                  />
                </div>
                <select
                  value={form.frequency}
                  onChange={(e) => setField('frequency', e.target.value as Frequency)}
                  className="flex-1 px-3 py-3.5 rounded-xl border-2 border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-900 text-gray-900 dark:text-white focus:outline-none focus:border-pink-400 transition-colors text-sm"
                >
                  {FREQS.map((f) => (
              ...
              <button
                ...
                className={`w-full py-3.5 rounded-xl font-bold text-sm transition-all ${
                  canSave
                    ? 'bg-pink-500 text-white shadow-sm active:scale-95'
        ...
        {incomeSources.length === 0 && !form && (
          <div className="text-center py-6">
            <p className="text-gray-400 text-sm">No income added yet.</p>
            <p className="text-gray-400 text-xs mt-1">You can skip this and fill it in later.</p>
          </div>
        )}
      </div>
    </OnboardingStepCard>
